...
def _estimate_mttr_minutes(selected_scenario: str, status: str, alarm_count: int) -> int:
    """シナリオごとに基準値を持たせたMTTR推定（モックだが一貫性のあるAIエージェント風）"""
    if status not in ["停止", "要対応", "注意"]:
        return 0

    # シナリオ名ベースの基準（必要に応じて拡張）
    # 目安: 完全停止(広域) > 完全停止(局所) > 要対応 > 注意
    baseline_by_scenario = {
        "WAN全回線断": 240,   # 広域断は長め
        "FW片系障害": 45,     # 冗長性でサービス継続しやすい
    }

    base = baseline_by_scenario.get(selected_scenario, 60)
    ...
    # アラーム数で微調整（暴れないように上限）
    extra = min(120, max(0, alarm_count) * 4)
    return base + extra
...
def _build_company_rows(selected_scenario: str):
    ...
    for tenant_id, network_id in all_scopes:
        ...
        # MTTR推定（シナリオ基準値 + 状態 + アラーム数）
        mttr_min = _estimate_mttr_minutes(selected_scenario, status, alarm_count)
        mttr = f"{mttr_min}分" if mttr_min > 0 else "-"
...
def _render_all_companies_board(selected_scenario: str, df_height: int = 220):
    ...
    with tab1:
        ...
        if not PLOTLY_AVAILABLE:
            ...
            total_notice = sum(1 for r in rows if r['status'] == '注意')
            # 健全性スコア（停止/要対応/注意 を反映）
            overall_health = 100 - (total_critical * 50 + total_warning * 25 + total_notice * 10)
            ...
            if len(df_plot) > 0:
                ...
                # 選択処理（Plotly traceごとのindexずれ対策：customdata を参照）
                try:
                    if selected_points and hasattr(selected_points, "selection") and selected_points.selection:
                        sel = selected_points.selection
                        # Streamlit PlotlyState: sel has `points` (list)
                        points = getattr(sel, "points", None)
                        if points:
                            cd = points[0].get("customdata") if isinstance(points[0], dict) else None
                            if cd and len(cd) >= 2:
                                tenant_id, network_id = cd[0], cd[1]
                                st.session_state.selected_scope = {"tenant": tenant_id, "network": network_id}
                        else:
                            # Fallback: some versions expose point_indices only (per-trace). We avoid it when multi-trace.
                            pass
                except Exception:
                    pass
    with tab2:
        ...
        if filtered_rows:
            ...
            for idx, r in enumerate(filtered_rows):
                with st.container():
                    ...
                    # アクションボタン
                    with cols[4]:
                        # 「詳細を表示」：選択スコープをセットし、下段のコックピットへ（アンカーリンク）
                        if st.button("📋", key=f"detail_{r['tenant']}_{r['network']}", help="詳細を表示"):
                            st.session_state.selected_scope = {"tenant": r['tenant'], "network": r['network']}
                        st.markdown(
                            '<div style="margin-top:6px;"><a href="#aiops-incident-cockpit" style="text-decoration:none;">詳細を表示</a></div>',
                            unsafe_allow_html=True
                        )
                    
                    with cols[5]:
                        if r['status'] in ["停止", "要対応", "注意"]:
                            if st.button("🚀 クイック修復", key=f"action_{r['tenant']}_{r['network']}", type="primary", help="クイック修復"):
                                st.session_state.selected_scope = {"tenant": r['tenant'], "network": r['network']}
                                st.session_state.auto_remediate = True
                                st.session_state.auto_remediate_forced = True
                                st.session_state.quick_fix_notice = (
                                    f"✅ **クイック修復を開始**\n"
                                    f"- 対象: **{display_company(r['tenant'])} / {r['network']}**\n"
                                    f"- 方針: 詳細分析（長文レポート）はスキップし、修復プラン生成を先行\n"
                                    f"- 期待動作: 直下の *Remediation & Chat* に修復手順が生成されます\n"
                                )
...
st.markdown('<div id="aiops-incident-cockpit"></div>', unsafe_allow_html=True)
...
with col_chat:
    ...
    # クイック修復の実行内容を表示（オペレーターの不安を減らす）
    if st.session_state.get("quick_fix_notice"):
        st.info(st.session_state.quick_fix_notice)
    ...
    if st.session_state.get("auto_remediate"):
        ...
        forced = bool(st.session_state.get("auto_remediate_forced"))
        st.session_state.auto_remediate_forced = False

        # 通常: incident候補が十分確からしい場合に自動生成
        if (selected_incident_candidate and selected_incident_candidate.get("prob", 0) > 0.6) or forced:
            st.session_state.remediation_plan = "Auto-generating..."
    
    if ((selected_incident_candidate and selected_incident_candidate.get("prob", 0) > 0.6) or ("remediation_plan" in st.session_state)):
        ...
        if "remediation_plan" in st.session_state:
            if st.session_state.remediation_plan == "Auto-generating...":
                with st.spinner("自動修復プランを生成中..."):
                    if not api_key:
                        st.session_state.remediation_plan = "⚠️ API Keyが未設定のため、修復プラン生成を実行できません。"
                    else:
                        # incident候補が無い場合でも、選択スコープとシナリオから最も近いノードを仮定
                        if selected_incident_candidate:
                            node_id = selected_incident_candidate.get("id")
                            cause_label = selected_incident_candidate.get("label", "Quick Fix")
                        else:
                            node_id = _find_target_node_id(TOPOLOGY, node_type="ROUTER") or _find_target_node_id(TOPOLOGY, node_type="FIREWALL")
                            cause_label = f"Quick Fix ({selected_scenario})"

                        t_node = TOPOLOGY.get(node_id) if node_id else None
                        plan_md = generate_remediation_commands(f"Cause: {cause_label}", t_node, api_key)
                        st.session_state.remediation_plan = plan_md
                    st.rerun()
            else:
                with st.container(border=True):
