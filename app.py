...
def _render_all_companies_board(selected_scenario: str, df_height: int = 220):
    """
    完全プロフェッショナル版: Plotlyヒートマップ + インタラクティブトリアージ
    """
    ...
    # タブ切り替え
    tab1, tab2, tab3 = st.tabs(["🔥 ヒートマップ", "📊 トリアージ・コマンドセンター", "📈 トレンド分析"])
    
    with tab1:
        # 2. Plotlyヒートマップ（インタラクティブ）
        st.markdown("### 🔥 全社ステータス・ヒートマップ")
        st.caption("アラーム数とステータスを一目で把握 | クリックで詳細表示")
        
        if not PLOTLY_AVAILABLE:
            st.warning("⚠️ Plotlyがインストールされていません。簡易版ヒートマップを表示します。")
            # Plotly未インストール時のフォールバック
            cols_per_row = 4
                ...
                for col_idx in range(cols_per_row):
                    ...
                    with cols[col_idx]:
                        ...
                        if st.button(
                            ...
                            key=f"heat_fallback_{r['tenant']}_{r['network']}",
                            use_container_width=True,
                            help=f"クリックで{r['company_network']}の詳細を表示"
                        ):
            ...
            # Plotlyが利用可能な場合
            # データ準備
            companies = [r['company_network'] for r in rows]
            ...
            status_values = []
            for s in statuses:
                if s == "停止": status_values.append(100)
                elif s == "要対応": status_values.append(75)
                elif s == "注意": status_values.append(50)
                else: status_values.append(10)
            ...
            for i, r in enumerate(rows):
                ...
                # 色の計算（アラーム数とステータスの組み合わせ）
                if r['status'] == "停止":
                    color_val = 100
                elif r['status'] == "要対応":
                    color_val = 75 + min(r['alarm_count'] / 2, 25)
                elif r['status'] == "注意":
                    color_val = 30 + min(r['alarm_count'], 20)
                else:
                    color_val = 5
                ...
                text = f"<b>{r['company_network']}</b><br>{icon} {r['alarm_count']}件"
                if r['delta'] and r['delta'] != 0:
                    if r['delta'] > 0:
                        ...
                        text += f"<br>▼{r['delta']}"
                texts.append(text)
                custom_data.append([r['tenant'], r['network'], r['alarm_count'], r['status']])
        
            
            # Scatter plotでヒートマップ風に
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    ...
                    colorbar=dict(
                        title="深刻度",
                        tickmode='array',
                        ticktext=['正常', '注意', '要対応', '停止'],
                        tickvals=[10, 40, 75, 100],
                    ),
                ...
                hovertemplate='<b>%{customdata[0]}社</b><br>' +
                             'ステータス: %{customdata[3]}<br>' +
                             'アラーム: %{customdata[2]}件<br>' +
                             '<extra></extra>',
            ))
            ...
            if selected_points and selected_points.selection.points:
                ...
                # Handle both dict and int types for point data
                if isinstance(point_data, dict):
                    # Extract index from dict if present
                    point_index = point_data.get('pointIndex', point_data.get('point_index', 0))
                else:
                    point_index = point_data
                
                # Ensure point_index is valid
                if isinstance(point_index, (int, float)) and 0 <= int(point_index) < len(rows):
                    selected_company = rows[int(point_index)]
                    st.session_state.selected_scope = {
                        "tenant": selected_company['tenant'],
                        "network": selected_company['network']
                    }
                    st.rerun()
    
    with tab2:
        # 3. プロフェッショナル・トリアージ・コマンドセンター
        ...
        with col2:
            filter_alarm = st.slider(
                "アラーム数",
                0, max(alarm_counts) if alarm_counts else 50,
                (0, max(alarm_counts) if alarm_counts else 50),
                key="filter_alarm"
            )
        with col3:
        ...
        if filtered_rows:
            ...
            for idx, r in enumerate(filtered_rows):
                with st.container():
                    ...
                    # アラーム情報
                    with cols[2]:
                        st.metric("アラーム", f"{r['alarm_count']}件", 
                                 f"▲{r['delta']}" if r['delta'] and r['delta'] > 0 else 
                                 f"▼{r['delta']}" if r['delta'] and r['delta'] < 0 else None)
                    
                    # MTTR
                    with cols[3]:
                        st.metric("推定MTTR", r['mttr'])
                    
                    # アクションボタン
                    with cols[4]:
                        if st.button("📋 詳細", key=f"detail_{r['tenant']}_{r['network']}"):
                            st.session_state.selected_scope = {
                                "tenant": r['tenant'],
                                "network": r['network']
                            }
                    ...
                    with cols[5]:
                        if r['status'] in ["停止", "要対応"]:
                            if st.button("🚀 対応", key=f"action_{r['tenant']}_{r['network']}", 
                                       type="primary"):
                                st.session_state.selected_scope = {
                                    "tenant": r['tenant'],
                                    "network": r['network']
    ...
    with tab3:
        ...
        if not PLOTLY_AVAILABLE:
            st.info("📊 Plotlyがインストールされていないため、トレンドグラフは表示できません。")
        ...
        # 統計情報（Plotlyの有無に関わらず表示）
        col1, col2, col3 = st.columns(3)
        with col1:
            st.info(f"**本日の総アラーム数**: {sum(alarm_counts)}件")
        with col2:
            avg_mttr = "45分"  # モック
